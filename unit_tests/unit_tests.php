...
$class_dirs = array_diff(scandir('./classes/'), array('.', '..', '.svn'));
...
foreach ($class_dirs as $class_dir) {
	...
	foreach ($class_tests as $class_test) {
		...
		preg_match_all('#^(ok|not ok) (\d+) - (?:Error: )?(\w+)\((\w+)\)$#ims', $result, $matches, PREG_SET_ORDER);
		...
		foreach ($matches as $match) {
			...
			if ($match[1] != 'ok') {
				list($error_message) = (array) $xml->testsuite->testcase[($match[2]-1)]->error;
				$lines = explode("\n", $error_message);
