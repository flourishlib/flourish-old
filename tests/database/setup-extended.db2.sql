...
CREATE TABLE other_user_details (
	id INTEGER NOT NULL PRIMARY KEY REFERENCES users(user_id) ON DELETE CASCADE,
...
CREATE TABLE record_deals (
	record_label VARCHAR(255) NOT NULL REFERENCES record_labels(name) ON UPDATE NO ACTION ON DELETE CASCADE,
	artist_id INTEGER NOT NULL REFERENCES artists(artist_id) ON DELETE CASCADE,
	PRIMARY KEY (record_label, artist_id)
);

CREATE TABLE favorite_albums (
	email VARCHAR(200) NOT NULL REFERENCES users(email_address) ON UPDATE  NO ACTION ON DELETE CASCADE,
...
CREATE TABLE top_albums (
	top_album_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE event_details (
	...
	allows_registration CHAR(1) NOT NULL CHECK(allows_registration IN ('0', '1'))
);
